...
func main() {
	...
	if *dirFlag != "" {
		...
		// Detect GitHub Actions or CI environment
		if os.Getenv("CI") != "" || os.Getenv("GITHUB_ACTIONS") != "" {
			fmt.Fprintln(os.Stderr, "Error: --dirname must be provided in CI")
			os.Exit(1)
		}

		// Local use with GUI prompts
	...
	if *dirFlag == "" {
		zenity.Notify("Folder "+selectedFolder+" was indexed properly", zenity.Title("Information"),
			zenity.InfoIcon)
	}
